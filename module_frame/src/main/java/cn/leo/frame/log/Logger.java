...
public class Logger extends FrameLayout implements Thread.UncaughtExceptionHandler, Application.ActivityLifecycleCallbacks {
    /**
     * 正式环境(false)不打印日志，也不能唤起app的debug界面
     */
                ...
                if (me == null) {
                    ...
                    Looper.myQueue().addIdleHandler(new MessageQueue.IdleHandler() {
                        ...
                        public boolean queueIdle() {
                            //线程异常处理设置为自己
                            Thread.setDefaultUncaughtExceptionHandler(me);
                            return false;
                        }
                    });
                }
    ...
    public void onActivityResumed(Activity activity) {
        ...
        if (debuggable) {
            if (checkIgnore(activity) || !showWindow) {
                return;
            }
    ...
    private boolean handleException(final Thread t, final Throwable e) {
        ...
        if (null == mCurrentActivity) {
            openWebView("http://127.0.0.1:45678");
        }
        new Thread() {
            @Override
            public void run() {
                ...
                if (null == mCurrentActivity) {
                    showInWeb(crashLog, t, e);
                    return;
                }
                Spanned spanned = Html.fromHtml(crashLog);
                Looper.prepare();
    ...
    private String createHtml(CharSequence msg) {
        ...
        public String checkLeak() throws InterruptedException {
            ...
            for (Activity activity : mMap.keySet()) {
                ...
                if (!mList.contains(s)) {
                    ...
                    i(name + "<-如果多次看到这个提示，则可能发生内存泄漏,请检查");
                }
            }
            mQueue.remove();
